...
public class CustomViewBehind extends ViewGroup {
	...
	protected void onLayout(boolean changed, int l, int t, int r, int b) {
		...
		if (mSecondaryContent != null)
			mSecondaryContent.layout(0, 0, width - mSecondaryWidthOffset,
					height);
	}

	/**
	 * �޸�secondary��width, 2013-12-19
	 */
	@Override
	...
	public void scrollBehindTo(View content, int x, int y) {
		...
		if (mMode == SlidingMenu.LEFT) {
			...
			if (x <= content.getLeft()) {
				...
				scrollTo( // �ұ߿��
						(int) (getSecondaryBehindWidth() - getWidth() + (x - getSecondaryBehindWidth())
								* mScrollScale), y);
			}
		}
	...
	public int getMenuLeft(View content, int page) {
		if (mMode == SlidingMenu.LEFT) {
			...
			case 0:
				return content.getLeft();
			case 2:
				return content.getLeft() + getBehindWidth(); // �ұ߿��
			}
		} else if (mMode == SlidingMenu.LEFT_RIGHT) {
			switch (page) {
			case 0:
				return content.getLeft() - getBehindWidth();
			case 2:
				return content.getLeft() + getSecondaryBehindWidth(); // �ұ߿��
			}
		}
		return content.getLeft();
	}
	...
	public void drawFade(View content, Canvas canvas, float openPercent) {
		...
		if (mMode == SlidingMenu.LEFT) {
			...
			left = content.getRight();
			right = content.getRight() + getBehindWidth();// �ұߵĿ��
		} else if (mMode == SlidingMenu.LEFT_RIGHT) {
			left = content.getLeft() - getBehindWidth();
			right = content.getLeft();
			canvas.drawRect(left, 0, right, getHeight(), mFadePaint);
			left = content.getRight();
			right = content.getRight() + getSecondaryBehindWidth();// �ұߵĿ��
		}
		canvas.drawRect(left, 0, right, getHeight(), mFadePaint);

		Log.d("BehindDrawFade",
				"on drawFade mFadePaint = " + mFadePaint.getAlpha());
	}
	 ...
	 * ---------�Զ�����չ Secondary Width---------------
	 */
	private int mSecondaryWidthOffset; // �ұ߿��ƫ����
