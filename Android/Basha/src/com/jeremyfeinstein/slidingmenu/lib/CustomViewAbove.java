...
public class CustomViewAbove extends ViewGroup {
	...
	protected float getPercentOpen() {
		Log.d("getPercentOpen", "mScrolX = " + mScrollX);
		Log.d("getPercentOpen", "mScrollX - mContent.getLeft = "
				+ (mScrollX - mContent.getLeft()));

		//如果是左右都能滑动，则往右滑时，使用getSecondaryBehindWidth
		if (mViewBehind.getMode() == SlidingMenu.LEFT_RIGHT) {
			float offset = mScrollX - mContent.getLeft();
			if (offset > 0) { //右滑
				return offset / getSecondaryBehindWidth();
			} else if (offset < 0) {//左滑
				return Math.abs(offset) / getBehindWidth();
			}
