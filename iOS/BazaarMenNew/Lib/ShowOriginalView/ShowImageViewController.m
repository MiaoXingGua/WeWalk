...
- (id)initWithFrame:(CGRect)frame ImageUrl:(NSString *)imageUrl Image:(UIImage *)image
{
	...
	if (self = [super init])
    {
        ...
        _activityView.center = CGPointMake(160, self.view.frame.size.height/2);
...
- (void)initImageView
{
    ...
    scrollView = [[UIScrollView alloc] initWithFrame:CGRectMake(0.0, 0.0, 320.0, self.view.frame.size.height)];
    scrollView.backgroundColor = [UIColor clearColor];
    scrollView.delegate = self;
    scrollView.maximumZoomScale = 2.0;
    scrollView.center = CGPointMake(160, self.view.frame.size.height/2);
    ...
    if(_imageView.frame.size.height>=self.view.frame.size.height)
    {
        _imageView.center = CGPointMake(160, _imageView.frame.size.height/2);
    }
    else
    {
        _imageView.center = CGPointMake(160, self.view.frame.size.height/2);
    }
...
- (void)tap:(UIGestureRecognizer *)gesture
{
    [UIView animateWithDuration:0.3 delay:0 options:UIViewAnimationOptionCurveLinear animations:^{
        ...
        [[NSNotificationCenter defaultCenter] postNotificationName:REMOVECOVERIMAGEVIEW object:nil];
...
- (void)scrollViewDidEndZooming:(UIScrollView *)scrollView withView:(UIView *)view atScale:(float)scale
{
    ...
    {
        [UIView animateWithDuration:0.2 delay:0 options:UIViewAnimationOptionCurveLinear animations:^{
            _imageView.center = CGPointMake(_imageView.center.x, self.view.frame.size.height/2);
