...
- (id)initWithFrame:(CGRect)frame arrowImageName:(NSString *)arrow textColor:(UIColor *)textColor
{
    ...
    if (self) {
		...
		UILabel *label = [[UILabel alloc] initWithFrame:CGRectMake(0.0f, loadViewHight - 20.0f, self.frame.size.width, 20.0f)];
...
//		label.shadowOffset = CGSizeMake(0.0f, 1.0f);
		...
		label = [[UILabel alloc] initWithFrame:CGRectMake(0.0f, loadViewHight - 38.0f, self.frame.size.width, 20.0f)];
		label.autoresizingMask = UIViewAutoresizingFlexibleWidth;
		label.font = curFont12;
		label.textColor = textColor;
//		label.shadowColor = [UIColor colorWithWhite:0.9f alpha:1.0f];
...
- (void)setState:(EGOPullLoadState)aState
{
	...
	switch (aState) 
    {
		case EGOOPullLoadPulling:
			...
			_statusLabel.text = NSLocalizedString(@"即将开始加载", nil);
		...
		case EGOOPullLoadNormal:
			...
			_statusLabel.text = NSLocalizedString(@"显示更多", nil);
            _statusLabel.font = [UIFont systemFontOfSize:15];
