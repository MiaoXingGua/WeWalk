...
-(void)relayoutDisplaySubviews
{
    ...
    if (!self.tableviews) {
        ...
        self.tableviews = [NSMutableArray arrayWithCapacity:_columnCount];
        
        for (int i = 0; i < _columnCount; i++)
        {
            UITableView *tableView = [[UITableView alloc] initWithFrame:CGRectMake(self.cellWidth*i, 0, self.cellWidth, CGRectGetHeight(self.frame)) style:UITableViewStylePlain];
...
//    }
    ...
    for (UITableView *tabelview in self.tableviews)
    {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    CGFloat cellHeight = [self.waterFlowViewDelegate waterFlowView:self heightForRowAtIndexPath:_indextPath];
    CGRect cellRect = CGRectMake(3, 5, self.cellWidth-6, cellHeight);
