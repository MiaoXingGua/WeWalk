...
- (void)getNotificationsOfThreadCountWithType:(ALThreadNotificationType)type
                                     isUnread:(BOOL)isUnread
                                        block:(void(^)(NSInteger count, NSError *error))resultBlock
{
    if (![User currentUser]) return;
...
+ (void)refreashNotificaitionWithBlock:(void(^)(NSDictionary *resultDictionary))resultBlock
{
    ...
    if (![ALBazaarEngine defauleEngine].user) return;
    ...
    [threadNotificationQ whereKey:@"toUser" equalTo:[ALBazaarEngine defauleEngine].user];
    ...
    [friendNotificationQ whereKey:@"toUser" equalTo:[ALBazaarEngine defauleEngine].user];
...
- (void)_friendNotificationHandle:(NSNotification *)notification
{
    ...
    ALFriendNotificationType type = [userInfo[@"ALFriendNotificationType"] integerValue];
...
//    [dic setValue:@"message.wav" forKey:@"sound"];
    ...
    switch (type)
    {
        case ALFriendNotificationTypeOfNewMessage:
        {
            [dic setValue:@"您收到一条新的私信" forKey:@"alert"];
        }
            break;
        case ALFriendNotificationTypeOfNewFollower:
        {
            [dic setValue:@"您有一位新的粉丝" forKey:@"alert"];
        }
            break;
        default:
        {
